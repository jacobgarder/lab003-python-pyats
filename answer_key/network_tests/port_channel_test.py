...
class portchannel_checks(aetest.Testcase):
    ...
    def setup(self, testbed):
        ...
        for device_name, device in testbed.devices.items():
            ...
            if device.os in ("ios", "iosxe"):
                ...
                self.learnt_portchannels[device_name] = device.parse("show etherchannel summary")
    ...
    def lacp_test(self, testbed, steps):
        ...
        for device_name, device in testbed.devices.items():
            ...
            with steps.start(
                f"Verifying port-channel protocol on {device_name}.",
                continue_=True,
            ) as device_step:
                ...
                for port_channel in topology_portchannels:
                    ...
                    with device_step.start(
                        ...
                        try:
                            ...
                            if learnt_details["protocol"].lower() != "lacp":
                                portchannel_step.failed(f"{port_channel} running protocol {learnt_details['protocol']}")
                        except KeyError:
                            # TODO: If no port-channel protocol is found, the port-channel step should "error"
                            portchannel_step.errored(f"{port_channel} no protocol information found.")
    ...
    def portchannel_flagtest(self, testbed, steps):
        ...
        for device_name, device in testbed.devices.items():
            # TODO: Every device should be ran as individual Steps. A failure on one device should NOT prevent testing of subsequent devices
            with steps.start(
                f"Verifying port-channels 'Up' flags on {device_name}.",
                continue_=True,
            ) as device_step:
                # Learn interface details for the device
                learnt_portchannels = self.learnt_portchannels[device_name]["interfaces"]
                        ...
                        with device_step.start(
                            ...
                            try:
                                if "U" not in learnt_details["flags"]:
                                    ...
                                    portchannel_step.failed(
                                        f"{interface} 'U' flag missing. Flags are: '{learnt_details['flags']}'."
                                    )
                            except KeyError:
                                # TODO: If no flag information is found for the port-channel, the port-channel step should "error"
                                portchannel_step.errored(f"{interface} no flag information found.")
    ...
    def portchannel_memmbers_test(self, testbed, steps):
        ...
        for device_name, device in testbed.devices.items():
            # TODO: Every device should be ran as individual Steps. A failure on one device should NOT prevent testing of subsequent devices
            with steps.start(
                f"Verifying port-channels members {device_name}.",
                continue_=True,
            ) as device_step:
                ...
                for interface, details in device.interfaces.items():
                    if hasattr(details, "channel_group"):
                        try:
                            ...
                            logger.error(
                                f"Port-channel{details.channel_group} not defined in testbed for {device_name}"
                            )
                ...
                for port_channel, topology_members in topology_portchannels.items():
                    ...
                    with device_step.start(
                        ...
                        if "members" in learnt_details.keys():
                            ...
                            if set(learnt_members) != set(topology_members):
    ...
    @aetest.test
    def portchannel_memmbers_flagtest(self, testbed, steps):
        """
        ...
        for device_name, device in testbed.devices.items():
            # TODO: Every device should be ran as individual Steps. A failure on one device should NOT prevent testing of subsequent devices
            with steps.start(
                f"Verifying port-channels member flags {device_name}.",
                continue_=True,
            ) as device_step:
                # Find the learnt port-channel details for this device
                learnt_portchannels = self.learnt_portchannels[device_name]["interfaces"]
                ...
                for interface, details in device.interfaces.items():
                    ...
                    if hasattr(details, "channel_group"):
                        ...
                        with device_step.start(
                            f"Checking port-channel member {interface}",
                            continue_=True,
                        ) as member_step:
                            ...
                            member_flags = Dq(learnt_portchannels).contains(interface).get_values("flags")
                            ...
                            #   if not fail step
                                ...
                                member_step.failed(
                                    f"Port-channel member-interface {interface} has a flag of {member_flags[0]}"
                                )
